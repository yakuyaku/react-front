...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${FASTAPI_BACKEND_URL}/api/v1/posts/${queryString ? `?${queryString}` : ''}`;
    ...
    return NextResponse.json(
      { detail: 'Failed to connect to posts service' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${FASTAPI_BACKEND_URL}/api/v1/posts/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
